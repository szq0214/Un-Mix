...
# Modified by Zhiqiang Shen (http://zhiqiangshen.com/) for Un-Mix.
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    losses_mix = AverageMeter('Loss_mix', ':.4e')
    losses_mix_flip = AverageMeter('Loss_mix_flip', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, losses_cl, losses_mix, losses_mix_flip, top1, top5],
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        # compute output
        output, target, output_m, output_m_flip = model(im_q=images[0], im_k=images[1], im_qm = mixed_images, im_qm_flip = mixed_images_flip, lam = lam)
        ...
        losses_cl.update(loss_o.item(), images[0].size(0))
        losses_mix.update(loss_m.item(), images[0].size(0))
        losses_mix_flip.update(loss_m_flip.item(), images[0].size(0))
